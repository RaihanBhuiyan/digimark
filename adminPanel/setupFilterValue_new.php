...
<div class="card-body">
		<form action="Filter-Value" method="post" enctype="multipart/form-data" autocomplete="off">
			<div class="row">
			    ...
			    // 	 	<option value=''>--Select Category--</option>";
			  	...
			  	<div class="col-md-6 col-sm-12 col-xs-12 mt-3">
			  		<p><b>Filter Head</b></p>
			  		<div id="filterValue">
			  			<select name="fhead" class="form-control select2" id="fhead" onchange="filter_for_head(this.value)">
								<option value="">--Select Filter Group--</option>
			  			<?php   
			  					$sql  = "SELECT `f_head_code`,`f_head_name` FROM `master_filter_head`";
					    		$result = mysqli_query($dbcon,$sql) or die ('error 404');
									while($row = mysqli_fetch_assoc($result)){
			    ...
			    <div class="col-md-6 col-sm-12 col-xs-12 mt-3">
			    	...
			    	<input type="text" name="filter_box" id="filter_box" style="display: none">
			  		...
			  		<input type="text" name="edit_code" id="edit_code" style="display: none">
				</div>
			...
			<div class="col-12" style="margin-top: 2.5%">
                <button type="submit" name="submit" id="submit" class="btn btn-success btn-md">Save <i class="fa fa-check"></i></button>
                <button type="submit" name="update" id="update" class="btn btn-warning btn-md" disabled="true">Update <i class="fa fa-check"></i></button>
...
if(isset($_POST['update'])){
	...
	$sqlget1 = "UPDATE `master_filter_value` SET `status` = '$status',`crDate` = '$crDate',`crBy` = '$crBy',`filter_value`  = '$value' WHERE `filter_code` = '$edit_code'";
