		...
		<form action="Credentials" method="post" enctype="multipart/form-data">
			<div class="row">
		  		...
		  		<div class="col-md-12 col-sm-12 col-xs-12 mt-3">
			    	...
			    	<input type="text" name="code" value="<?php echo $_SESSION['userCode'];?>" class="form-control" style="display: none">
		    	</div>
		  		<div class="col-md-12 col-sm-12 col-xs-12 mt-3">
...
<?php
 if(isset($_POST['submit'])){
    ...
    $sql3 = "SELECT * FROM `master_user` WHERE `userCode` = '$userCode'";
	$result3 = mysqli_query($dbcon,$sql3) or die ('error 404');
			        while($row3 = mysqli_fetch_assoc($result3)){
			            $oldPassword = $row3['userPassword'];
			        }

	if($oldPassword == $prevPassword)
		{
			$sql = "UPDATE `master_user` SET `userPassword` = '$newPassword',`crDate` = '$cr_date',`crBy` = '$cr_by' WHERE `userCode` = '$userCode'";
