...
<div class="card">
<div class="card-header"><h1>Manage Newsletter</h1></div>
<div class="card-body">
		<form action="Manage-Newsletter" method="post" enctype="multipart/form-data">
			<div class="row">
		  		...
		  		<div class="col-md-12 col-sm-12 col-xs-12 mt-3">
			    	...
			    	<textarea required name="link" class="form-control" placeholder="Link"><?php echo $news_link;?></textarea>
			    </div>
        ...
        //       				else{ echo '<option value="1">Active</option>
			...
			<div class="col-md-12 col-sm-12 col-xs-12" style="text-align: right;margin-top: 2.5%" id="submitRow">
                ...
                <a id="showList" onclick="getList('Manage-Newsletter')" class="btn btn-info btn-md">List <i class="fa fa-bars"></i></a>
                <a id="showList" onclick="reload('Manage-Newsletter')" class="btn btn-info btn-md">Clear <i class="fa fa-refresh"></i></a>
...
<?php
 if(isset($_POST['submit'])){
    ...
    $news_link = addslashes($_POST['link']);
	...
	if(strlen($code)>0){
	    ...
	    if($news_status == 2){
	            $sql  = "UPDATE `tbl_newsletter` SET `status` = '1' WHERE `status` = '2'";
                $result = mysqli_query($dbcon,$sql) or die ('error 404');
	        }  
	        
			$sql  = "UPDATE `tbl_newsletter` SET `title` = '$news_title', `url` = '$news_url', `details` = '$news_details', `link` = '$news_link',  `metaTitle` = '$metaTitle',  `metaDesc` = '$metaDesc',  `metaKeyWord` = '$metaKeyWord', `status` = '$news_status',`crDate` = '$crDate',`crBy` = '$crBy' WHERE `code` = '$code'";
			$result = mysqli_query($dbcon,$sql) or die ('error 404');

			if($_FILES['image']['size'] > 0){

		 	$image_prev = $_POST['image_prev'];
			unlink('../assets/images/newsletter/'.$image_prev);
		...
		$setName = $name.'.'.$fileextension;
		    $sql = "UPDATE `tbl_newsletter` SET `thumbnail` = '$setName' WHERE `code` = '$code'";
	 	$result = mysqli_query($dbcon,$sql) or die ('error 4041');
		}
		
			echo "<script>location='Manage-Newsletter'</script>";
		}
	    ...
	    $sql = "INSERT INTO `tbl_newsletter`(`id`, `code`, `title`, `url`, `details`,`link`,`metaTitle`,`metaDesc`,`metaKeyWord`, `status`, `crDate`, `crBy`) VALUES ('', '', '$news_title','$news_url', '$news_details', '$news_link','$metaTitle','$metaDesc','$metaKeyWord', '$news_status', '$crDate', '$crBy')";
	...
	$sqlget1 = "UPDATE `tbl_newsletter` SET `code` = '$newId' WHERE `id` = '$last_id'";
	...
	if($_FILES['image']['size'] > 0){
		...
		$setName = $name.'.'.$fileextension;
		    $sql = "UPDATE `tbl_newsletter` SET `thumbnail` = '$setName' WHERE `code` = '$newId'";
	 	$result = mysqli_query($dbcon,$sql) or die ('error 4041');
		}
...
function dispImage(value){
	//alert(value);
	MyWindow=window.open('Display-Image?image='+value+'&item=newsletter','MyWindow','width=300,height=300'); return false;
}
</script>
