  	...
  	<div class="col-12" style="border:1px solid lightgray">
	    <div class="row row-product-inner-top">
			<div class="col-md-6 col-sm-12 text-md-left text-center" style="width:100%;margin: 0px !important">
					...
					<a style="color:#f16724;text-decoration:none" href="qcollections/<?php echo $litePath[2];?>"><?php echo ucfirst($litePath[2]).'</a>';
					if($litePath[3] !== null){
		...
		<div class="col-lg-2 col-md-3 col-sm-12 col-12" style="border:1px solid lightgray;background-color: transparent;padding: 0.5% !important;padding-top: 1% !important;">
			...
			<div id="filter-box" class="d-none d-md-block" style="width: 100%;">
				...
				// 		echo '<b style="font-size:18px;color:#000000;margin-top:2.5%;margin-bottom:2.5%;line-height:6vh">'.$filter_head.'</b><br>';
					...
					$sql_filter = "SELECT DISTINCT `master_filter_value`.`filter_code` AS `filter_code`,`master_filter_value`.`filter_value` AS `filter_value`
					FROM `master_filter_value` 
					...
					LEFT JOIN `tbl_category` ON `master_filter`.`category` = `tbl_category`.`category_code` 
					WHERE `master_filter_head`.`f_head_name` = '$filter_head' AND `master_filter`.`category` LIKE '%$filter_head_category%'"; 
					//sub category
...
function choosedItem(code){
		...
		localStorage.setItem("row_"+rowSelected+"_category", document.getElementById("selected_category_"+code).value);
		localStorage.setItem("row_"+rowSelected+"_subcategory", document.getElementById("selected_subcategory_"+code).value);
