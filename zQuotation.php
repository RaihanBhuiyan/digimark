...
<div class="col-12" style="min-height:30vh;margin-bottom:3%">
    ...
    <div class="col-md-2 col-sm-2 col-6">Subcategory</div>
    ...
    <div class="col-md-1 col-sm-2 col-6">Image</div>
      ...
      $("#clr").click(function(){
        ...
        locItem = parseInt(localStorage.getItem("getID"));
            for(i=1;i<=locItem;i++){
                    ...
                    localStorage.setItem(("row_"+i+"_subcategory"),'Empty');
...
<script type="text/javascript">
    function getSubcategory(value,val){
        //alert(value+'--'+val);
        $.ajax({
              type:"post",
              url:"ajaxGetSubCategoryQuotation.php",
              data:{category:value,id:val},
              success:function(data)
              {
                if(!data){
                  document.getElementById('subcategoryList'+val).style.display="none";
                }else{
                  if(data !== ''){
                    //alert(data);
                  document.getElementById('subcategoryList'+val).style.display="block";
                  $('#subcategoryList'+val).html(data);
                  }else{
                    //alert(data);
                  document.getElementById('subcategoryList'+val).style.display="none";
                  }
    ...
    locItem = parseInt(localStorage.getItem("getID"));
            for(i=1;i<=locItem;i++){
                ...
                if(localStorage.getItem("row_"+i+"_category")!== 'Empty')
                {
                 ...
                 $("#product-container").append('<div class="row" id="row_'+i+'" style="margin-top:1%"><div class="col-md-2 col-sm-2 col-6">'+localStorage.getItem("row_"+i+"_category")+'</div><div class="col-md-2 col-sm-2 col-6">'+localStorage.getItem("row_"+i+"_subcategory")+'</div><div class="col-md-2 col-sm-4 col-6"><label id="comp_name">'+localStorage.getItem("row_"+i+"_name")+'</label></div><div class="col-md-1 col-sm-4 col-6"><label id="comp_qty">'+localStorage.getItem("row_"+i+"_qty")+'</label></div><div class="col-md-2 col-sm-2 col-6"><label id="comp_uprice">'+localStorage.getItem("row_"+i+"_price")+'</label></div><div class="col-md-1 col-sm-2 col-6"><label id="comp_tprice">'+localStorage.getItem("row_"+i+"_tprice")+'</label></div><div class="col-md-1 col-sm-2 col-6"><img id="comp_image" src="'+localStorage.getItem("row_"+i+"_image")+'" alt="No Image" style="width:50px"></div><div class="col-md-1 col-sm-2 col-6"><a href="javascript:void(0)" class="btn btn-sm" id="action_button" onclick="remove_item('+i+')" style="background-color: red;border-radius: 0px;color:white;font-weight: bold">X</a></div></div><hr style="border-bottom:2px solid #1b4e90">');
                }
...
function openTab(code){
    ...
    var subcategory = document.getElementById('row_subcat_'+currentRow+'').value;
    //alert(category);
    
     window.open('qcollections/'+subcategory+'/','blank');
}
