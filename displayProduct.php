...
$sql = "SELECT *,`tbl_product`.`cr_date` AS `pcrdate`,`tbl_brand`.`brand_name` AS `brand`,`tbl_category`.`category_name` AS `category`,`tbl_category`.`category_url` AS `category_url`  FROM `tbl_product` LEFT JOIN `tbl_brand` ON `tbl_product`.`product_brand` = `tbl_brand`.`brand_code` 
...
$i = 0;
	while($row = mysqli_fetch_assoc($result)){
	    $i++;
			...
			}elseif($product_status == '0' && $product_redirect_url == ''){
			    echo "<script>location='../../page-not-found'</script>";
			}
	}
if($i == 0){
...
LEFT JOIN `tbl_category` ON `tbl_product`.`product_category` = `tbl_category`.`category_code` WHERE `tbl_product`.`product_redirect_url` = '$product_url'";
$result = mysqli_query($dbcon,$sql) or die ('error 404');
$i = 0;
	while($row = mysqli_fetch_assoc($result)){
			$reviewProduct = $product_code = $row['product_code'];
			...
			$product_status = $row['product_status'];
			$product_redirect_url = $row['product_redirect_url'];
			if($product_status == '1' && $product_url !== ''){
				echo "<script>location='/product/".$product_url."'</script>";
			}
	}
}
?>
...
<div class="container" style="margin-top:0.5%">
	<div class="row" style="">
		<div class="col-12" style="padding:1% !important;background-color:#ffffff;color:#000000 !important;font-size:14px;">
				...
				if(mysqli_num_rows($result_mtnc) <= 0){
					...
					{$i = 0;
						while($row_mtnc = mysqli_fetch_assoc($result_mtnc)){
							...
							if($i==1){
								echo '<img itemprop="image" class="xzoom" id="xzoom-default" src="assets/images/products/'.$image.'" xoriginal="assets/images/products/'.$image.'" alt="'.$product_name.'" style="height:100% !important" />
									<div class="xzoom-thumbs">
										<a href="assets/images/products/'.$image.'"><img itemprop="image" class="xzoom-gallery" src="assets/images/products/'.$image.'" alt="'.$product_name.'" title="The description goes here 2" style="height:100% !important"/></a>';
							}else{
								echo '<a href="assets/images/products/'.$image.'"><img itemprop="image" class="xzoom-gallery" src="assets/images/products/'.$image.'" alt="'.$product_name.'" title="The description goes here 3" style="height:100% !important"/></a>';
							}
						}
					}
	...
	<div class="col-md-7 col-sm-12 col-xs-12" style="padding-left: 5%;padding-top: 2% !important;padding-bottom: 2% !important;"  itemscope itemtype="https://schema.org/Product">
		<h1 class="chone" style=""  itemprop="name"><?php echo $product_name;?></h1>
    	<meta itemprop="sku" content="<?php echo $product_code;?>" />
    	<meta itemprop="description" content="<?php echo $product_metaDescription;?>" />
		...
		<td class="product-info-label" style="width:10%;font-weight:bold"><b>Price</b></td>
		<td class="product-info-data product-price" style="width: 25%;" >
		...
		<td class="product-info-label" style="width:10%;font-weight:bold"><b>Status</b></td>
		<td class="product-info-data product-status" style="width: 25%;">
			<?php 
				if($stock_status == 1){
				echo '
				<span>In stock</span>';
		...
		<tr class="product-info-group">
		<td class="product-info-label" style="width:10%;font-weight:bold"><b>Product Code</b></td>
		<td class="product-info-data product-code" style="width: 25%;">
			
			<?php echo $product_code;?></td>
			<span itemprop="mpn" style="display: none">not inserted</span>
		</tr>
		<tr class="product-info-group">
		<td class="product-info-label" style="width:10%;font-weight:bold"><b>Brand</b></td>
		<td class="product-info-data product-brand" style="width: 25%;">
			<span><?php echo $product_brand;?></span></td>
		</tr>
		<tr class="product-info-group">
		<td class="product-info-label" style="width:10%;font-weight:bold"><b>Model</b></td>
		<td class="product-info-data product-model" style="width: 25%;">
		...
		<td>
		<h6 style="color:#f16724;margin-top:2.5%"><b>Features</b></h6>
		...
		<td>
		<h6 style="color:#f16724"><b><?php echo $product_remark;?></b></h6>
		</td>
		</tr>
		<tr style="border-bottom: none;">
		<td>
		<?php 
		if($product_brochure != null){echo '<a class="btn btn-sm btn-dark" href="assets/images/product_brochure/'.$product_brochure.'" rel="nofollow" download="'.$product_name.'" style="margin-left:0% !important"><i class="fas fa-download"  style="color:red"></i>  Download Brochure</a>';}
		else{echo "No Brochure";}
		?>
		</td>
		</tr>
		<td style="padding-top:3.5%">
		<h7 style="color:#f16724;paddin-bottom:2.5%"><b>Share On :</b></h7><br>
		<!--<button class="btn btn-xs btn-primary">Facebook <i class="fas fa-share"></i></button>-->
		<!--<div class="fb-share-button" -->
  <!--      data-href="https://www.your-domain.com/your-page.html" -->
  <!--      data-layout="button_count">-->
  <!--      </div>-->
  <div class="sharethis-inline-share-buttons" style="margin-top:2%;text-align:left !important"></div>
		</td>
		</tr>
...
<div class="row" style="margin-bottom: 2.5%;">
	...
	<div class="col-12" id="p_large_desc" style="text-align: left;padding-top: 0.5% !important;padding-bottom: 0.5% !important" itemscope itemtype="https://schema.org/Article">
        <?php echo '<span style="color:#f16724">Last Modified On: </span><span itemprop="dateModified">'.date("d-m-Y", strtotime($product_cr_date)).'<span><br>';
...
// 			$specHead = $row2['spec_head_code'];
    ...
    //             echo '<div class="col-lg-3 col-md-3 col-sm-4 col-12" style="text-align: center;margin-top:1%;margin-bottom:1%">
    ...
    //             </div>';
	   //     }
	   	...
	   	$product_url = $litePath[2];
	   // $sql = "SELECT *,`tbl_brand`.`brand_name` AS `brand`  FROM `tbl_product` LEFT JOIN `tbl_brand` ON `tbl_product`.`product_brand` = `tbl_brand`.`brand_code` WHERE `tbl_product`.`product_category` = '$product_category' AND `product_url` != '$product_url' AND `product_code` IN(SELECT `product_code` FROM `master_filter` WHERE `category` = '$product_category' AND `f_head_code` = 'FHD00001' AND `filter_code` IN (SELECT `filter_code` FROM `master_filter` WHERE `category` = '$product_category' AND `f_head_code` = 'FHD00001' AND `product_code`='$product_code')) ORDER BY RAND() LIMIT 4";//ORDER BY `tbl_product`.`product_code` ASC LIMIT 4";
	   $sql = "SELECT * from `tbl_product` where `product_code` IN(select `product_code` from `tbl_product` where `product_subcategory` IN (select `product_subcategory` from `tbl_product` where  `product_url` LIKE '%$product_url%')) ORDER BY RAND() LIMIT 4";
        $result = mysqli_query($dbcon,$sql) or die ('error 404');
	        while($row = mysqli_fetch_assoc($result)){
			    $product_code_thumb = $row['product_code'];
			    $product_brand_thumb = $row['brand'];
			    $product_name_thumb = $row['product_name'];
			    $product_price_thumb = $row['product_price'];
			    $product_thumb = $row['product_thumb'];
			    $product_url_thumb = $row['product_url'];
			
                echo '<div class="col-lg-3 col-md-3 col-sm-4 col-12" style="text-align: center;margin-top:1%;margin-bottom:1%">
                  <div class="divLink" style="margin:auto !important;border:1px solid lightgray;height:100%;">
                    <a href="product/'.$product_url_thumb.'/">
                        <div id="pcontainer" style="background-color:lightgray;margin:0px !important;height:100% !important;font-size:14px;">
                            <img class="img img-responsive" src="assets/images/product_thumb/'.$product_thumb.'" alt="'.$product_thumb.'"><br>
                            <label style="margin-top:-5% !important">'.$product_name_thumb.'</label>
                        </div>
                    </a>
                  </div>
                </div>';
